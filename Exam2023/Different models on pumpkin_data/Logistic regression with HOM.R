...
set.seed(123)  
pumpkin_split <- initial_split(pumpkin, prop = .7, strata = "Color")
pumpkin_train <- training(pumpkin_split) 
pumpkin_test  <- testing(pumpkin_split) 

# Two simple LR
model1 <- glm(Color ~ Item.Size, 
              family = "binomial", 
              data = pumpkin_train)
model2 <- glm(Color ~ City.Name, 
              family = "binomial", 
              data = pumpkin_train)
...
set.seed(123)
cv_model2 <- train(
  Color ~ City.Name+Item.Size, #added
  data = pumpkin_train, 
  method = "glm",
  family = "binomial",
  trControl = trainControl(
...
pred_class_test <- predict(cv_model4, pumpkin_test) #response 'class' 
pred_class_test
pred_prob <- predict(cv_model4, pumpkin_test,type = "prob")$ORANGE # probability
pred_prob

# confusion matrix and statistics
confusionMatrix(
  data = pred_class_test, 
  reference = pumpkin_test$Color)
...
# building ROC
library(ROCR)
m4_prob <- predict(cv_model4, pumpkin_test, type = "prob")$WHITE  # predicted prob.
perf4 <- prediction(m4_prob, pumpkin_test$Color) 
...
auc_ROCR_model4
#[1] 0.9720268 ! it may depend on your split


# For comparison,one can represent the ROC curves for the three best models in the same plot 
m3_prob <- predict(cv_model3, pumpkin_test, type = "prob")$WHITE
perf_m3 <- prediction(m3_prob, pumpkin_test$Color) 
roc_ROCR_model3 <-  performance(perf_m3, measure = "tpr", x.measure = "fpr")

mpls_prob <- predict(cv_model_pls, pumpkin_test, type = "prob")$WHITE
perf_pls <- prediction(mpls_prob, pumpkin_test$Color) 
